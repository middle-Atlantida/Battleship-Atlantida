...
export class ButtleScreen {
    ...
    constructor(
        ...
        this.shipsAvailable = [
            this.player.shipCounter.map(({ counter }) => counter),
            this.opponent.shipCounter.map(({ counter }) => counter),
        ];
    }
    ...
    renderShipsCounter = async () => {
        const players = [{ ...this.player } as Battlefield, { ...this.opponent } as Battlefield];
    ...
    isPlayerTurn = (event: Event) => {
        ...
        if (
            offsetX < OPPONENT_SCREEN_START_FIELD_COORD_X ||
        ...
        const cellWithShot = this.opponent.battlefield.flat().find(cell =>
            isOverElement(
                {
                    x: offsetX,
                    y: offsetY,
    ...
    updateCoordOpponentField() {
        this.opponent.battlefield.forEach(row =>
            row.forEach(cell => {
